...
import com.qiyue.utils.datetime.DateUtil;
import com.qiyue.utils.sys.OsUtil;
...
public class FileUploadUtil  {
    ...
    public static String upload(MultipartFile multipartFile,String fileType) throws IllegalStateException {
        ...
        switch (fileType)
        {
            ...
            case "AUDIO"://类型为音频
                if (OsUtil.isWindows()) {
                    sb.append(FilePath.AUDIO_FILE_UPLOAD_WIN_PATH);
                } else {
                    sb.append(FilePath.AUDIO_FILE_UPLOAD_LUX_PATH);
                }
                break;
            default:
                return  null;
        ...
        if (OsUtil.isWindows()) {
            substr = sb.toString().substring(FilePath.IMG_FILE_UPLOAD_WIN_PATH.length() + File.separator.length() + File.separator.length(), sb.toString().length());
        } else {
            substr = sb.toString().substring(FilePath.IMG_FILE_UPLOAD_LUX_PATH.length() + File.separator.length() + File.separator.length(), sb.toString().length());
        }
